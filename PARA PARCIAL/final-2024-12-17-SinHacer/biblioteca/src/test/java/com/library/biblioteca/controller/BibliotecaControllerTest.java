...
class BibliotecaControllerTest {
    ...
    void setUp() {
        ...
        registro.setNombreCliente("Registro 1");
        registro.setLibrosReservados(Arrays.asList(libro));
    ...
    void testVerTodosLosLibros() throws Exception {
        ...
        mockMvc.perform(get("/api/biblioteca/libros"))
                ...
                .andExpect(jsonPath("$.[0].isbn").value("193129132"))
    ...
    void eliminarLibro() throws Exception {
        ...
        mockMvc.perform(delete("/api/biblioteca/libros/{id}", libroId))
                .andExpect(status().isOk());
    ...
    @Test
    void verTodosLosAlquileres() throws Exception{
        when(bibliotecaService.verTodosLosAlquileres()).thenReturn(registros);

        mockMvc.perform(get("/api/biblioteca/alquilar"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.size()").value(1))
                .andExpect(jsonPath("$.[0].nombreCliente").value("Registro 1"))
                .andExpect(jsonPath("$[0].librosReservados.size()").value(1));
    }
